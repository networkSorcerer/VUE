<template>
    ...
    <div class="container">
      <div class="row">
        <div class="col">
          ...
          <div>
            <select class="form-select" v-model="ChLec" style="height: 50px;width: 200px;" @change="CounselList($event)">
  ...
  <CD
    ...
    :lectureId="lectureId"
  />
<Pagination v-bind="{currentPage , totalItems  : totalCnt, itemsPerPage : 10}" @search="CounselList($event)" v-if="dataList.length > 0"/>
</template>
...
const CounselList = (cpage) => {
  ...
  param.append('lec_id',ChLec.value); // 선택된 값을 사용
