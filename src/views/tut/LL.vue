<template>
  <div> 
    <div> 
        ...
        <div class="row">
            <LC v-for="data in dataList" :key="data.lecrm_id" :data="data"/>
        </div>
...
const searchLecture = async(cpage)=>{
    ...
    const letureList = await axiosAction(SamplePage5.LectureRoomList, param);

    if(letureList) {
        dataList.value = letureList.listdata;
        total.value = letureList.listcnt;
